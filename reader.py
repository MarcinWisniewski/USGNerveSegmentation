...
class Reader(object):
    def __init__(self, img_path, minibatch, train_size=0.8):
        '''
        :param img_path: path to train or test images
        :param minibatch: number of images in minibatch (must be multiplication of 64 )
        '''
        assert minibatch % IMAGE_MULTIPLIER**2 == 0, ('minibatch must be multiplication of %d' % IMAGE_MULTIPLIER**2)
    ...
    def _get_images(self, image_type, img_iterator):
        ...
        multiplied_img = np.zeros(np.append(self.minibatch, crop_img_shape))
        number_of_images_to_read = self.minibatch/IMAGE_MULTIPLIER**2
        multiplied_mask = np.copy(multiplied_img)
        transformed_multiplied_mask = np.zeros((self.minibatch, 5))
        img_num = 0
            ...
            for y_axis in xrange(IMAGE_MULTIPLIER):
                for x_axis in xrange(IMAGE_MULTIPLIER):
                    ...
                    multiplied_mask[img_num] = mask[y_axis:y_axis + crop_img_shape[0],
                                               x_axis:x_axis+crop_img_shape[1]]
                    temp_transformed_multiplied_mask = self.mask2ellipse.convert(mask[y_axis:y_axis + crop_img_shape[0],
                                                                                 x_axis:x_axis+crop_img_shape[1]])[2]
                    transformed_multiplied_mask[img_num] = self._convert_tuples_to_list(temp_transformed_multiplied_mask)
                    img_num += 1

        return multiplied_img, multiplied_mask, transformed_multiplied_mask, img_iterator
